...
- 🎯 **Repo plane rescoped to prose the principal reads — RULED and DELIVERED
      ...
      *"I accept your recommendation"* — keep the engine and reply gate
      ...
      One verdict closes both this pointer and the enforcement-clause pointer in
      item `300-…`. **CMF1 (MAJOR):** the reply plane's premise is false by the
      ...
      *Delta:* `tools/plainscan.py`, `tools/test_plainscan.py`, `tools/floor.py`,
      `docs/method/COMMUNICATION.md` § *The meta-rules that make it work*
      (`e390382`). *Intent record:* this item.
