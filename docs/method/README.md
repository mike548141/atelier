...
4. **`CONCURRENCY.md`** — one worktree per line of work; serialise real-world
   side-effects.
5. **`TOOLBOX.md`** — keep a tool manifest; approved-but-missing may be
   installed; keep the personal inventory machine-local.
6. **`PRINCIPLES.md`** — the design principles + precedence ladder + situation
   tests. *(Extraction in progress — canonical source is ros `docs/PRINCIPLES.md`.)*
7. **`MODEL-ECONOMICS.md`** — which model does what, and what it costs.
